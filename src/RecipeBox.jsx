...
export default function RecipeBox({ onSignOut }) {
  ...
  async function handleAddNationalDish(dish) {
    ...
    try {
      ...
      setErrorMsg(`Heads up: this recipe was generated by Claude for "${dish.dish}" rather than extracted from a real source — a good starting point, but worth double-checking quantities and technique.`);
      setAddStage('review');
    } catch (err) {
      const msg = err?.message || '';
      if (msg.startsWith('NETWORK')) setErrorMsg(`Network error: ${msg.replace('NETWORK: ', '')}`);
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', paddingRight: '18px' }}>
                    ...
                    <div>
                      ...
                      <button
                        onClick={() => handleAddNationalDish(todaysDish)}
                        style={{
                          display: 'inline-flex', alignItems: 'center', gap: '5px', marginTop: '9px',
                          background: 'none', border: `1px solid ${COLORS.sage}`, color: COLORS.sage,
                          borderRadius: '3px', padding: '5px 10px', fontSize: '11.5px', fontWeight: 600, cursor: 'pointer',
                        }}
                      >
                        <Plus size={12} /> Add to Recipe Box
                      </button>
                    </div>
                  </div>
                </div>
